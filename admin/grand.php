...
$sql = "SELECT F.id, t1.team t1, t2.team t2, F.round, F.block, F.next_block, F.id_t1, F.id_t2 
    FROM $dbt_grand F
...
$id_last_round = $conn -> query ("SELECT round, block FROM $dbt_grand
...
for($i=1;$i<=$last_round['round'];$i++)
{
    ...
    foreach($final as $value)
    {
        ...
        if($value['round'] == $i)
        {
            ...
            $url = "<a href='?id=".$id_game."&id_match=".$value['id']."&round=".$value['round']."&block=".$value['block']."&next_block=".$value['next_block']."&id_t1=".$value['id_t1']."&id_t2=".$value['id_t2']."&t1=".$value['t1']."&t2=".$value['t2']."#grandEdit'>";
