...
if($_POST['s1']){
    ...
    if($_POST)
    {
        $stmt = $conn->prepare("UPDATE `q_games` SET `s1`=?, `s2`=? WHERE (`id`=?)"); 
    ...
    {
        global $conn, $s1, $s2, $id_game, $id_q1, $id_q2;
        $stmt = $conn->prepare("UPDATE `qualification` SET `r$round`=?, `d$round`=? WHERE (`id_game`=?) AND (`id_team`=?)"); 
    ...
    $stmt = $conn->prepare("UPDATE `qualification` SET `result`=?, `difference`=? WHERE (`id_game`=?) AND (`id_team`=?)");
    ...
    $result = $conn->query("SELECT Sum(IFNULL(r1,0)+IFNULL(r2,0)+IFNULL(r3,0)) AS result, 
                            ...
                            FROM qualification WHERE id_team = $id_q1 AND id_game = $id_game");
    ...
    $result = $conn->query("SELECT Sum(IFNULL(r1,0)+IFNULL(r2,0)+IFNULL(r3,0)) AS result, 
                                Sum(IFNULL(d1,0)+IFNULL(d2,0)+IFNULL(d3,0)) AS difference
                            FROM qualification WHERE id_team = $id_q2 AND id_game = $id_game");
...
if($_POST['table']){
    $table = $_POST['table'];
    $stmt = $conn->prepare("UPDATE `q_games` SET `table`=? WHERE (`id`=?)"); 
